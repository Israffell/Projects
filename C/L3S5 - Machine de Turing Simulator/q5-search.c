...
int MT_SEARCH(int argc, char* argv[], MT mt, int Ki, int Kj){
	
    if(argc < 3){printf("Il manque un/des argument(s).");exit(1);}          //Il y a 1 txt et 2 bandes minimum
    if(argc > 11){ printf("Il y a un/des argument(s) en trop.");exit(1);}    //Pouvant aller jusqu'à 10 (k=10)
      
    //printf("\n  Test nombre argc: %d\n",argc);
	
	char temp_file[13] = {0};
	char* txt0 ="SEARCH-a0.txt";
	char* txt1="SEARCH-a1.txt";
	
	if(Ki == 0){ strcpy(temp_file, txt0);}
	if(Ki == 1){ strcpy(temp_file, txt1);}
